...
fn main() {
    let matches = App::new("NFT Image Generator")
        ...
        .subcommand(App::new("build_examples")
    ...
    if let Some(ref matches) = matches.subcommand_matches("build") {
        ...
        if matches.is_present("debug") {
            ...
            let all_images_in_layers_iterator = layers_content.iter().map(|x| x.iter()).multi_cartesian_product();
            println!("Images in layers are generated. Starting composing\n");
            images_generator::generate_and_merge_images(
                &collection_schema,
                all_images_in_layers_iterator).expect("Composition failed.");

            let usage_examples_schema = images_generator::read_usage_generation_schema(
                "usage_generation_schema.json").unwrap();
            images_generator::generate_usage_examples(
                usage_examples_schema).expect("Failed to generate sample images.");
        }
    }
