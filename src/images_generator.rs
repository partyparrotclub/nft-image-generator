...
pub fn get_trait_name_from_image_path(image_path: &std::path::PathBuf) -> Result<String> {
...
pub fn generate_and_merge_images(
    layers: &Vec<Layer>,
    all_images_in_layers_iterator: MultiProduct<std::slice::Iter<std::path::PathBuf>>,
) -> Result<()> {
    let mut i_image = 0;
    ...
    for image_to_generate in all_images_in_layers_iterator {
        ...
        for i_layer in 1..image_to_generate.len() {
            ...
            let trait_value = NFTAttribute {
                ...
                value: get_trait_name_from_image_path(image_to_generate[i_layer]).unwrap(),
            };
        ...
        to_writer(
            &File::create(format!("images/{}.json", i_image)).unwrap(),
            &NFTMetadata,
        )?;
        i_image += 1;
    }
...
pub fn generate_usage_examples(usage_examples: Vec<ImageUsageExample>) -> Result<()> {
    ...
    for usage_example in usage_examples {
        ...
        let mut template_file = image::open(format!("templates/{}.png", usage_example_filename))
            .expect("Opening template file failed");
        ...
        for placement in usage_example.placements {
            ...
            let file_name = format!("images/{}.png", i_image);
            let pfp_image = image::open(file_name)
                .expect("Opening sample pfp failed. Make sure the file exists.");
        ...
        template_file
            .save(format!("images/examples/{}.png", usage_example_filename))
            .expect("Failed to save image.");
    }
